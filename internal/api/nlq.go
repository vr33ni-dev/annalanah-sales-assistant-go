...
// fmt and strings are still used by generateSQL below
...
func (h *Handler) RunNLQ(w http.ResponseWriter, r *http.Request) {
	...
	v, err, _ := sqlCache.group.Do(sqlText, func() (interface{}, error) {
		if os.Getenv("NLQ_MOCK") == "1" {
			resp := nlqResponse{SQL: sqlText, Columns: []string{}, Rows: []map[string]interface{}{}}
		...
		cols, results, err := h.store.ExecuteRawQuery(ctx, sqlText)
		if err != nil {
		...
		sqlCache.Set(sqlText, resp)
		log.Printf("Executed NLQ SQL in %v: %s", time.Since(start), sqlText)
		return resp, nil
	})
	if err != nil {
		writeJSONErr(w, nlqResponse{Error: err.Error()}, err)
