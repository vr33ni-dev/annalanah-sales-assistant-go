...
IMPORT_SALES_TABLE ?= sales_metrics

.PHONY: import-cashflow import-salesprocess import-all
...
import:
	...
	if [ "$${TYPE}" = "sales" ] || [ "$${TYPE}" = "both" ]; then \
			if [ "$${MODE}" = "preview" ] || [ "$${MODE}" = "dryrun" ]; then \
				go run ./tools/importsales -csv="$${SALESFILE}" -dryrun -verbose; \
			else \
				if [ -z "$(DATABASE_URL)" ]; then echo "DATABASE_URL missing"; exit 1; fi; \
				echo "Persisting sales CSV into domain tables (clients/sales_process/contracts/upsells)"; \
				go run ./tools/importsales -csv="$${SALESFILE}" -dsn="$(DATABASE_URL)" -persist; \
				echo "Running extractor to produce ambiguous_names.csv for manual review"; \
				go run ./tools/extract_sales_names -csv="$${SALESFILE}" -out=ambiguous_names.csv -dsn="$(DATABASE_URL)"; \
...
import-all:
	...
	if [ "$${MODE}" = "dryrun" ]; then \
		...
		if [ -z "$(DATABASE_URL)" ]; then echo "DATABASE_URL missing; set it in .env or export it in your shell"; exit 1; fi; \
			echo "import-all: persisting cashflow and sales into domain tables, then processing"; \
			$(MAKE) import TYPE=both MODE=persist; \
	fi
...
# convenience alias: fill executes the full persist + staging processing (like your previous "make fill")
.PHONY: fill
fill:
	@$(MAKE) import-all MODE=auto
