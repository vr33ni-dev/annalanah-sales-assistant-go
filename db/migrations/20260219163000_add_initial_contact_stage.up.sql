-- Add 'initial_contact' stage and 'initial_call_scheduled' client status
-- Drops the old CHECK constraints (if present) and replaces them with expanded sets.
...
ALTER TABLE IF EXISTS sales_process
  ADD CONSTRAINT sales_process_stage_check CHECK (stage IN ('initial_contact','follow_up','closed','lost'));
